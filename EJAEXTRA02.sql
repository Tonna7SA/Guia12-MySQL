...
select codigo_cliente from pago
where fecha_pago = date_format(fecha_pago,'2008-%m-%d');
...
select * from pago
where forma_pago = 'paypal' and year(fecha_pago)=2008 order by total;
...
-- de ventas.
select cliente.nombre_cliente, empleado.nombre, empleado.apellido1, empleado.apellido2 from cliente inner join empleado
...
-- representantes de ventas.
select cliente.nombre_cliente, pago.*, empleado.nombre as 'representante' , empleado.apellido1, empleado.apellido2 from cliente inner join empleado
on empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas inner join pago on cliente.codigo_cliente = pago.codigo_cliente;

-- 3. Muestra el nombre de los clientes que no hayan realizado pagos junto con el nombre de 
-- sus representantes de ventas.

select cliente.nombre_cliente, empleado.nombre as 'representante' , empleado.apellido1, empleado.apellido2, pedido.estado, pedido.comentarios from cliente inner join empleado
on empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas inner join pedido on cliente.codigo_cliente = pedido.codigo_cliente
where pedido.comentarios = 'pago pendiente';

select distinct cliente.codigo_cliente ,cliente.nombre_cliente, empleado.nombre as 'representante' , empleado.apellido1, empleado.apellido2 from cliente inner join empleado
on empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas inner join pago on cliente.codigo_cliente != pago.codigo_cliente
where not cliente.codigo_cliente = any (select pago.codigo_cliente from pago);

-- 4. Devuelve el nombre de los clientes que han hecho pagos y el nombre de sus representantes 
-- junto con la ciudad de la oficina a la que pertenece el representante.
select cliente.nombre_cliente, pago.*, empleado.nombre as 'representante' , empleado.apellido1, empleado.apellido2, oficina.ciudad as 'ciudad oficina' 
from cliente inner join empleado on empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas
inner join pago on cliente.codigo_cliente = pago.codigo_cliente inner join oficina on empleado.codigo_oficina = oficina.codigo_oficina;

-- 5. Devuelve el nombre de los clientes que no hayan hecho pagos y el nombre de sus 
-- representantes junto con la ciudad de la oficina a la que pertenece el representante.
select cliente.nombre_cliente, empleado.nombre as 'representante' , empleado.apellido1, empleado.apellido2, oficina.ciudad as 'ciudad oficina', 
pedido.estado, pedido.comentarios from cliente inner join empleado on empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas
inner join pedido on cliente.codigo_cliente = pedido.codigo_cliente inner join oficina on empleado.codigo_oficina = oficina.codigo_oficina
where pedido.comentarios = 'pago pendiente';

-- 6. Lista la dirección de las oficinas que tengan clientes en Fuenlabrada.
select oficina.codigo_oficina, oficina.linea_direccion1, oficina.linea_direccion2, cliente.nombre_cliente, cliente.linea_direccion2, cliente.ciudad from oficina inner join empleado
on oficina.codigo_oficina = empleado.codigo_oficina inner join cliente on empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas
...
select cliente.nombre_cliente, empleado.nombre as 'nombre representante', empleado.apellido1, empleado.apellido2, oficina.ciudad as 'ciudad oficina' from cliente inner join empleado
on cliente.codigo_empleado_rep_ventas = empleado.codigo_empleado inner join oficina on empleado.codigo_oficina = oficina.codigo_oficina;

-- 8. Devuelve un listado con el nombre de los empleados junto con el nombre de sus jefes.
select empleado.codigo_empleado, empleado.nombre, concat(empleado.apellido1,' ',empleado.apellido2) as 'apellidos', empleado.codigo_jefe, 
(select concat(empleado1.nombre,' ',empleado1.apellido1,' ',empleado1.apellido2) 
from empleado as empleado1 where empleado.codigo_jefe = empleado1.codigo_empleado) as 'Nombre del Jefe' from  empleado;

-- 9. Devuelve el nombre de los clientes a los que no se les ha entregado a tiempo un pedido.
select cliente.nombre_cliente, pedido.fecha_esperada, pedido.fecha_entrega from cliente inner join pedido
on cliente.codigo_cliente = pedido.codigo_cliente
where datediff(fecha_entrega,fecha_esperada)>1;

-- 10. Devuelve un listado de las diferentes gamas de producto que ha comprado cada cliente.
select distinct cliente.nombre_cliente, gama_producto.gama from cliente inner join pedido 
on cliente.codigo_cliente = pedido.codigo_cliente inner join detalle_pedido
on pedido.codigo_pedido = detalle_pedido.codigo_pedido inner join producto
on detalle_pedido.codigo_producto = producto.codigo_producto inner join gama_producto
on producto.gama = gama_producto.gama order by cliente.nombre_cliente;
...
select cliente.* from cliente left join pago
...
select cliente.* from pedido right join cliente
on cliente.codigo_cliente = pedido.codigo_cliente 
...
select cliente.* from pedido right join cliente
on cliente.codigo_cliente = pedido.codigo_cliente left join pago
on cliente.codigo_cliente = pago.codigo_cliente  
...
-- asociada.
select empleado.* from empleado right join oficina
...
select empleado.* from empleado left join cliente
on empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas
...
select empleado.* from empleado right join oficina
on empleado.codigo_oficina = oficina.codigo_oficina right join cliente
on empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas
where not oficina.codigo_oficina is null and cliente.codigo_empleado_rep_ventas is null;


-- 7. Devuelve un listado de los productos que nunca han aparecido en un pedido.
select producto.* from producto left join detalle_pedido
on detalle_pedido.codigo_producto = producto.codigo_producto left join pedido
...
-- Consultas resumen
-- 1. ¿Cuántos empleados hay en la compañia ?
select count(empleado.codigo_empleado) as 'Cantidad de empleados' from empleado;

-- 2. ¿Cuántos clientes tiene cada país?
select count(cliente.codigo_cliente) as 'Cantidad clientes', cliente.pais from cliente group by cliente.pais;

-- 3. ¿Cuál fue el pago medio en 2009?
select avg(pago.total) as 'Promedio pago 2009' from pago
where year(fecha_pago)=2009;

-- 4. ¿Cuántos pedidos hay en cada estado? Ordena el resultado de forma descendente por el 
-- número de pedidos.
select pedido.estado, count(pedido.estado) as 'Cantidad de pedidos' from pedido group by pedido.estado order by count(pedido.estado) desc;

-- 5. Calcula el precio de venta del producto más caro y más barato en una misma consulta.
select producto.nombre, producto.precio_venta as 'Producto mas caro y mas barato' from producto
where producto.precio_venta = (select min(producto.precio_venta) from producto) or 
producto.precio_venta = (select max(producto.precio_venta) from producto) order by producto.precio_venta desc;

-- 6. Calcula el número de clientes que tiene la empresa.
select count(cliente.codigo_cliente) as 'Cantidad de clientes' from cliente;

-- 7. ¿Cuántos clientes tiene la ciudad de Madrid?
select count(cliente.codigo_cliente) as 'Cantidad de clientes de Madrid' from cliente
where cliente.ciudad = 'madrid' or cliente.region = 'madrid';

-- 8. ¿Calcula cuántos clientes tiene cada una de las ciudades que empiezan por M?
select cliente.ciudad, count(cliente.codigo_cliente) as 'Cantidad de clientes x ciudad' from cliente
where cliente.ciudad like 'm%'group by cliente.ciudad;

-- 9. Devuelve el nombre de los representantes de ventas y el número de clientes al que atiende 
-- cada uno.
select empleado.nombre, concat(empleado.apellido1,' ',empleado.apellido2) as 'apellidos', count(cliente.nombre_cliente) as 'cantidad clientes'
from empleado inner join cliente
on empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas 
where empleado.puesto = 'representante ventas' group by empleado.nombre, apellidos;

-- 10. Calcula el número de clientes que no tiene asignado representante de ventas.
select count(cliente.nombre_cliente) as 'Clientes sin representante' from cliente inner join empleado
...
select cliente.nombre_contacto, cliente.apellido_contacto, min(pago.fecha_pago) as 'Primer pago', max(pago.fecha_pago) as 'Ultimo pago' from cliente 
inner join pago on cliente.codigo_cliente = pago.codigo_cliente group by cliente.nombre_contacto, cliente.apellido_contacto; 

-- 12. Calcula el número de productos diferentes que hay en cada uno de los pedidos.
select detalle_pedido.codigo_pedido, count(detalle_pedido.codigo_producto<>detalle_pedido.codigo_producto) as 'Cantidad de productos'
from detalle_pedido group by detalle_pedido.codigo_pedido;

-- 13. Calcula la suma de la cantidad total de todos los productos que aparecen en cada uno de 
-- los pedidos.
select detalle_pedido.codigo_pedido, sum(detalle_pedido.precio_unidad*cantidad) as 'Suma total por pedido' from detalle_pedido 
...
-- unidades vendidas.
select detalle_pedido.codigo_producto, sum(detalle_pedido.cantidad) as 'Cantidad de productos vendidos' 
from detalle_pedido group by detalle_pedido.codigo_producto order by sum(detalle_pedido.cantidad) desc limit 20;
...
select sum(round(detalle_pedido.precio_unidad*cantidad)) as 'Base imponible', round(sum(detalle_pedido.precio_unidad*cantidad)*21/100) 
as '21%', round(sum(detalle_pedido.precio_unidad*cantidad)+(sum(detalle_pedido.precio_unidad*cantidad)*21/100)) as 'Total Facturado' from detalle_pedido; 

-- 16. La misma información que en la pregunta anterior, pero agrupada por código de producto.
select detalle_pedido.codigo_producto,  
round(sum(detalle_pedido.precio_unidad*cantidad)) as 'Base imponible',
round(sum(detalle_pedido.precio_unidad*cantidad)*21/100) as '21%', 
round(sum(detalle_pedido.precio_unidad*cantidad)+(sum(detalle_pedido.precio_unidad*cantidad)*21/100)) as 'Total Facturado' 
from detalle_pedido group by detalle_pedido.codigo_producto order by detalle_pedido.codigo_producto desc;
...
round(sum(detalle_pedido.precio_unidad*cantidad)*21/100) as '21%', 
round(sum(detalle_pedido.precio_unidad*cantidad)+(sum(detalle_pedido.precio_unidad*cantidad)*21/100)) as 'Total Facturado' 
from detalle_pedido where detalle_pedido.codigo_producto like 'or%'
...
-- IVA)
select detalle_pedido.codigo_producto, sum(detalle_pedido.cantidad) as 'Unidades vendidas',
round(sum(detalle_pedido.precio_unidad*cantidad)) as 'Subtotal',
round(sum(detalle_pedido.precio_unidad*cantidad)*21/100) as '21%', 
round(sum(detalle_pedido.precio_unidad*cantidad)+(sum(detalle_pedido.precio_unidad*cantidad)*21/100)) as 'Total Facturado' 
from detalle_pedido group by detalle_pedido.codigo_producto having Subtotal > 3000 order by Subtotal;

-- Subconsultas con operadores básicos de comparación
-- 1. Devuelve el nombre del cliente con mayor límite de crédito.
select cliente.nombre_cliente, cliente.limite_credito as 'Mayor credito' from cliente 
where cliente.limite_credito = (select max(cliente.limite_credito) from cliente);

-- 2. Devuelve el nombre del producto que tenga el precio de venta más caro.
select producto.nombre, producto.precio_venta as 'Mayor precio' from producto
...
select producto.codigo_producto, producto.nombre from producto inner join detalle_pedido
on producto.codigo_producto = detalle_pedido.codigo_producto
where detalle_pedido.cantidad = (select max(detalle_pedido.cantidad) from detalle_pedido);


select detalle_pedido.codigo_producto, sum(detalle_pedido.cantidad) as 'Cantidad de productos vendidos', producto.nombre 
from detalle_pedido inner join producto on detalle_pedido.codigo_pedido = producto.codigo_producto
group by producto.nombre, detalle_pedido.codigo_producto, sum(detalle_pedido.cantidad)
...
-- INNER JOIN).
select cliente.nombre_cliente, cliente.limite_credito, pago.total from cliente, pago
where cliente.codigo_cliente = pago.codigo_cliente  
and cliente.limite_credito > pago.total;

-- 5. Devuelve el producto que más unidades tiene en stock.
select producto.nombre, producto.cantidad_en_stock from producto
where producto.cantidad_en_stock = (select max(producto.cantidad_en_stock) from producto);

-- 6. Devuelve el producto que menos unidades tiene en stock.
select producto.nombre, producto.cantidad_en_stock from producto
where producto.cantidad_en_stock = (select min(producto.cantidad_en_stock) from producto);

-- 7. Devuelve el nombre, los apellidos y el email de los empleados que están a cargo de Alberto 
-- Soria.
select empleado.codigo_empleado, empleado.nombre, concat(empleado.apellido1,' ',empleado.apellido2) as 'apellidos', empleado.codigo_jefe, 
concat('Alberto', 'Soria') as 'Jefe' from empleado where empleado.codigo_jefe = (select empleado.codigo_empleado from empleado where 
empleado.nombre = 'Alberto' and empleado.apellido1 = 'Soria');


-- Subconsultas con ALL y ANY
-- 1. Devuelve el nombre del cliente con mayor límite de crédito.
select * from cliente
where cliente.limite_credito = any (select max(cliente.limite_credito) from cliente);

-- 2. Devuelve el nombre del producto que tenga el precio de venta más caro.
select * from producto
where producto.precio_venta = all (select max(producto.precio_venta) from producto);

-- 3. Devuelve el producto que menos unidades tiene en stock.
select * from producto
where producto.cantidad_en_stock = some (select min(producto.cantidad_en_stock) from producto);

-- Subconsultas con IN y NOT IN
-- 1. Devuelve el nombre, apellido1 y cargo de los empleados que no representen a ningún 
-- cliente.

select empleado.nombre, empleado.apellido1, empleado.puesto, cliente.codigo_empleado_rep_ventas, empleado.codigo_empleado from empleado left join
cliente on empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas
where cliente.codigo_empleado_rep_ventas is null;

select distinct empleado.nombre, empleado.apellido1, empleado.puesto, empleado.codigo_empleado from empleado
where empleado.codigo_empleado not in (select cliente.codigo_empleado_rep_ventas from cliente) order by codigo_empleado;  

-- 2. Devuelve un listado que muestre solamente los clientes que no han realizado ningún pago.

select cliente.nombre_cliente, cliente.codigo_empleado_rep_ventas from cliente left join pago
on cliente.codigo_cliente = pago.codigo_cliente
where pago.codigo_cliente is null;
...
where producto.codigo_producto not in (select detalle_pedido.codigo_producto from detalle_pedido where detalle_pedido.codigo_pedido 
in (select pedido.codigo_pedido from pedido));
...
select empleado.nombre, empleado.apellido1, empleado.apellido2, empleado.puesto, empleado.extension from empleado
where empleado.codigo_empleado not in (select cliente.codigo_empleado_rep_ventas from cliente)
 and empleado.puesto not in ('representante ventas');

-- Subconsultas con EXISTS y NOT EXISTS
-- 1. Devuelve un listado que muestre solamente los clientes que no han realizado ningún 
-- pago.
select cliente.nombre_cliente, cliente.codigo_cliente from cliente
where not exists (select * from pago where pago.codigo_cliente = cliente.codigo_cliente);

-- 2. Devuelve un listado que muestre solamente los clientes que sí han realizado ningún pago.
select cliente.nombre_cliente, cliente.codigo_cliente from cliente
where exists (select * from pago where pago.codigo_cliente = cliente.codigo_cliente);
